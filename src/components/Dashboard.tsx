...
const generateNtlData = () => {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    // Simulate a spike in the 3rd month of the sequence
    const intensity = i === 2 ? 85 : Math.floor(Math.random() * 15) + 20;
    data.push({
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm">
          ...
          <div className="flex items-center gap-1 text-amber-600 text-sm font-medium">
            ...
            <span className="text-zinc-400 font-normal ml-1">in {ntlData.find(d => d.intensity > 60)?.month || 'recent months'}</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-zinc-200 shadow-sm">
          ...
          <p className="text-sm text-zinc-500 mt-4">
            <span className="font-medium text-zinc-900">Analysis:</span> Sharp spike in {ntlData.find(d => d.intensity > 60)?.month || 'recent months'} correlates with reported night-shift activity.
          </p>
        </div>
      </div>
