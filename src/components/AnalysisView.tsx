            ...
            const result = await ai.models.generateContent({
              ...
              contents: {
                parts: [
                  ...
                  { text: "You are a moderation AI for an environmental satellite monitoring tool. First, check if the image is a satellite image, aerial view, map, or environmental data. If it is NOT (like a selfie, pet, or random object), reply EXACTLY with 'REJECTED|' followed by a funny, cute rejection message saying this is for environmental monitoring, not social media. If it IS valid, reply EXACTLY with 'ACCEPTED|' followed by a concise analysis of deforestation or construction." }
                ]
              }
            });
            
            const textResponse = result.text || "";
            if (textResponse.includes("REJECTED|")) {
              setError("🛑 " + textResponse.split("REJECTED|")[1].trim());
              setAnalyzing(false);
              return;
            }
            
            geminiExplanation = textResponse.replace(/.*ACCEPTED\|/s, "").trim() || textResponse;
          } catch (e) {
